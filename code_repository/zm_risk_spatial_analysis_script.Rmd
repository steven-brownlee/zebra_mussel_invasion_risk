...
cci_rds <- list.files('cci_lakes_rds', 
                            pattern = '*.rds',
                            full.names = TRUE,
                            recursive = TRUE)

date_frame <- tibble(date = character())
...
for (i in 5205:6940) {
  ...
  print(paste('Completed:', ival, 'of', 6940))
...
outdir <- 'cci_lakes_split_2/'

for (i in 1:1736) {
    ival <- eval(i)
    date_input <- date_frame %>% 
      filter(date_id == ival) %>% 
      deframe()
    file_selection <- cci_rds %>% str_subset(pattern = date_input)
    selection_basename <- basename(file_selection)
    newpath <- paste0(outdir, selection_basename)
    file.rename(from = file_selection,
                to = newpath)
    print(paste('Completed:', ival, 'of', 1736))
}
