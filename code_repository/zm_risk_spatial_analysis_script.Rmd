...
for (i in 1:length(cci_files)) {
  ...
  rw_443 <- interior_terra %>% 
   select(Rw443) %>% 
  crop(bc_bbox) %>% 
  st_as_stars() %>% 
   as_tibble()
  
  rw_443_coords <- turbidity_uncertainty %>% 
   select(x, y)
  
  ###
  
  rw_443_u <- interior_terra %>% 
   select(Rw443_uncertainty_relative) %>% 
  crop(bc_bbox) %>% 
  st_as_stars() %>% 
   as_tibble()
  
  rw_443_u_coords <- turbidity_uncertainty %>% 
   select(x, y)
  ...
  rw_547
   ...
   rw_547 <- interior_terra %>% 
   select(Rw547) %>% 
  crop(bc_bbox) %>% 
  st_as_stars() %>% 
   as_tibble()
  
   rw_547_coords <- turbidity_uncertainty %>% 
   select(x, y)
  
  ###
    
   
   rw_547_u <- interior_terra %>% 
   select(Rw547_uncertainty_relative) %>% 
  crop(bc_bbox) %>% 
  st_as_stars() %>% 
   as_tibble()
  
   rw_547_u_coords <- turbidity_uncertainty %>% 
   select(x, y)
  ...
  comb_table <- bind_rows(chla_mean, chla_uncertainty, 
                        ...
                        rw_443, rw_443_u, rw_547, rw_547_u)
