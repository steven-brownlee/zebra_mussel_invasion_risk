...
for (i in 1:length(cci_files)) {
  ...
  combined$clipping_group <- as.numeric(cut_number(combined$uid, 2000))
  
  interior_data_table <- tibble(x = numeric(),
                         y = numeric(),
                         date = character(),
                         chla_mean = numeric(),
  ...
  write_rds(interior_data_table, output_filename)
...
```{r}
lake_clipper <- function(clipping_group_number, cci_lakes,
                         combined) {
  
      gn_eval = eval(clipping_group_number)
      split <- combined %>% 
        filter(clipping_group == gn_eval)
      split_sf <- split %>% 
        st_as_sf(coords = c('x', 'y'),
                 crs = 4326) %>% 
        st_transform(4087) %>% 
        st_intersection(cci_lakes)
