...
for (i in 1:length(cci_files)) {
  ...
  coord_filling <- left_join(coord_filling, t_rw_560)
  
  coord_filling <- left_join(coord_filling, t_rw_560_u)
  ...
  interior_data_filled <- coord_filling %>%
    ...
    replace(is.na(.), 0) %>% 
    mutate(na_test = chla_mean + chla_uncertainty  + lswt_uncertainty + lswt_quality_level  + lwst + turbidity_mean + turbidity_uncertainty + rw_443 + rw_443_u + rw_560 + rw_560_u) %>% 
    filter(na_test != 0)
...
for (i in 1:length(cci_l_l)) {
  ...
  lake_rds$week <- floor_date(lake_rds$date, "week")
  
  lake_rds_grouped <- lake_rds %>% 
    group_by(x, y, point_id, week) %>%
  ...
  filename <- paste0('cci_lakes_by_lake_weekly/cci_', name_list[ival], '.rds')
  
  write_rds(lake_rds_grouped, filename)
  
  print(paste('Completed:', ival, 'of', length(cci_l_l)))
...
rds_interior <- lake_rds

date_sequence <- seq(dmy('01-01-2016'), dmy('31-12-2020'), by = 'week') 
...
cci_l_l <- list.files('cci_lakes_by_lake_weekly/', 
                            pattern = '*.rds',
                            full.names = TRUE,
                            recursive = TRUE)
### 
...
for (i in 1:length(cci_l_l)) {
  ...
  lake_filled_knn <- future_map(1:261,
                          ~nn_filler(.x, rds_interior = lake_rds)) %>% 
    bind_rows()
