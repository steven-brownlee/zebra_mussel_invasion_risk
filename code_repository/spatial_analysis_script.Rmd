...
## Import CCI lakes shapefile, match to frame

cci_lakes_shapefile <- read_sf('auxiliary_files/cci_lakes_corrected.gpkg') %>% 
  st_transform(st_crs(4087))

test <- st_intersection(calcium_data, cci_lakes_shapefile)


cci_lakes_test <- st_join(cci_lakes_shapefile, calcium_data) %>% 
 drop_na() 

cci_lakes_list <- unique(cci_lakes_test$name)

cci_lakes_shapefile <- cci_lakes_shapefile %>% filter(name %in% cci_lakes_list)

write_sf(cci_lakes_shapefile, 'auxiliary_files/cci_calcium_matchup_polygon.gpkg')
...
ems_lake_data <- list.files(path = 'in_situ_measurements/bc_lake_monitoring_program/lake_monitoring_raw_data/', 
                            pattern = '*.csv', 
                            full.names = TRUE) %>% 
...
ems_lake_data <- ems_lake_data %>% 
  mutate(id = rownames(ems_lake_data))
...
ems_vars <- ems_sf %>% filter(PARAMETER == 'Calcium Total' | 
                                   PARAMETER == 'Calcium Dissolved' |
                                   PARAMETER == 'Specific Conductivity-Field' |
                                   PARAMETER == 'Turbidity' |
                                   PARAMETER == 'Chlorophyll A' |
                                   PARAMETER == 'Temperature-Field')

# Retrieve epilimnion results

ems_vars <- ems_vars %>% filter(Level == 'Epilimnion')

# Filter data for lakes that have matching CCI datasets


ems_vars <- ems_vars %>% 
  st_transform(crs = st_crs(3005))

ems_vars <- st_intersection(ems_vars, cci_lakes_shapefile)

ems_vars <- ems_vars %>% 
  select(PARAMETER, COLLECTION_START, RESULT)


# Join EMS data to assembly frame
